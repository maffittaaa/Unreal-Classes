...
MyTCPClientRunnable::MyTCPClientRunnable(UMatchmakingSubsystem* InOwner)
 : Thread(nullptr)
 , Socket(nullptr)
 , bRun(true)
 , bConnected(false)
...
MyTCPClientRunnable::~MyTCPClientRunnable()
{
	Stop();
...
uint32 MyTCPClientRunnable::Run() {
...
void MyTCPClientRunnable::Stop() {
	bRun = false;
}

void MyTCPClientRunnable::SendRawString(const FString& Message) {
	if (!Socket)
		return;
...
void MyTCPClientRunnable::HostNewGame(const FString& Name, int32 Port) {
	if (!Socket)
		return;
...
void MyTCPClientRunnable::RequestSessionList() {
